(() => {
  ...
  var A11y = ({ swiper, extendParams, on }) => {
    ...
    const init = () => {
      ...
      initSlides();
      const nav = swiper.navigation ? swiper.navigation : { nextEl: void 0, prevEl: void 0 };
  ...
  // static/js/modules/hero-stats.js
  ...
  // static/js/modules/scroll-reveal.js
  var SCOPE_SELECTOR = ".page > header, .page > section, .page > footer, #page > header, #primary > section, #page > footer";
  var TARGET_SELECTOR = [
    ".section-title",
    ".hero__title",
    ".hero__description",
  ...
  var isEligible = (element) => element instanceof HTMLElement && !element.hidden && !element.closest("[hidden]") && !element.matches(".swiper-wrapper");
  var collectTargets = (scope) => [...scope.querySelectorAll(TARGET_SELECTOR)].filter(isEligible).sort((first, second) => {
    if (first === second) return 0;
    return first.compareDocumentPosition(second) & Node.DOCUMENT_POSITION_FOLLOWING ? -1 : 1;
  });
  var isCard = (element) => element.matches(
    ".problem-card, .metrics-card, .step-card, .review-card, .pricing-card, .service-card, .faq-item"
  );
  var revealTarget = (target, index) => {
  ...
  // static/js/modules/sticky-header.js
